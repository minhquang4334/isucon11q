...
module Isucondition
  class App < Sinatra::Base
    ...
    def generate_isu_graph_response(jia_isu_uuid, graph_date)
      rows = db.xquery('SELECT * FROM `isu_condition` WHERE `jia_isu_uuid` = ? AND `timestamp` >= ? AND `timestamp` <= ?  ORDER BY `timestamp` ASC', jia_isu_uuid, graph_date, graph_date + (3600*24))
